...
class SendEmailCodeForm(forms.Form):#验证码模块
    ...
    def clean_email(self):
        ...
        #
        if oneHourMaxClickNumber(no_valid_email+"_"):#一个小时内最大的点击次数
...
class EmailLoginForm(Base_Bootstrap): # 登录
    ...
    def clean_email(self):
        email = self.cleaned_data.get("email")
        user_obj = models.UserInfo.objects.filter(email=email).first()#省事
        if not user_obj:
...
class LoginForm(Base_Bootstrap):
